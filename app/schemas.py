...
class SensorDataIn(BaseModel):
    ...
    timestamp: Optional[
        datetime
    ]  # When the measurement was taken. It used insted of insertion time.
...
class AskResponse(BaseModel):
    ...
    llm_highlights: str
    sensors: Optional[List[SensorDataOut]] = (
        None  # The list of sensor data records returned by the LLM->SQL query, if any.
    )
    aggregation: Optional[str] = (
        None  # Native language output, textual aggregation result returned by the LLM->SQL query, if any.
    )
